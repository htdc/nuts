...
function Nuts(opts) {
    ...
    this.router.get('/download/channel/:channel/:platform/:tag?', this.onDownload);
    ...
    this.router.get('/update/channel/:channel', this.onUpdateRedirect);
    this.router.get('/update/channel/:channel/RELEASES', this.onUpdateRedirect);
...
Nuts.prototype.onUpdateRedirect = function(req, res, next) {
    ...
    .then(function() {
        ...
        if (baseUrl.endsWith('RELEASES')) {
            var basePath = baseUrl.substring(0, basePath.indexOf('RELEASES'))
            return res.redirect(basePath+'/'+req.query.platform+'/'+req.query.version+'/RELEASES');
        }
        else {
            return res.redirect(baseUrl+'/'+req.query.platform+'/'+req.query.version);
        }
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        // The url should specify the channel if it's provided
        var url;
        if (channel === '*')
            url = urljoin(fullUrl, gitFilePath, '/download/version/'+latest.tag+'/'+platform+'?filetype='+filetype)
        else
            url = urljoin(fullUrl, gitFilePath, '/download/channel/'+channel+'/'+platform+'/'+latest.tag+'?filetype='+filetype)
            ...
            releases = _.chain(releases)
                ...
                .map(function(entry) {
                    ...
                    if (channel === '*')
                        ...
                        entry.filename = urljoin(fullUrl, gitFilePath, '/download/channel/'+channel+'/'+platform+'/'+entry.semver+'?filetype=nupkg');
